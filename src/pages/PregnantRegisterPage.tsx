...
function SearchByCpf() {
  ...
  const handleSearch = async () => {
    ...
    setGestante(null);
    try {
      const response = await axios.get<IGestante>(`/api/gestantes/${cpf}`);
  ...
  return (
    <section className="bg-white p-6 rounded-lg shadow-md mb-8">
      ...
      <div className="flex flex-col sm:flex-row items-center gap-2 max-w-md mx-auto">
        ...
        />
        <button onClick={handleSearch} className="w-full sm:w-auto bg-[#1a5276] text-white py-2 px-4 rounded hover:bg-[#0e3040] transition-colors">
...
function RegistrationForm() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        try {
            await axios.post('/api/gestantes', formData);
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {Object.entries(formData).map(([key, value]) => (
                <div key={key} className="flex flex-col">
                    <label htmlFor={key} className="mb-1 font-semibold text-gray-700 capitalize">{key.replace(/_/g, ' ')}:</label>
                    <input
             ...
             <div className="md:col-span-2 text-center">
                <button type="submit" className="bg-[#1a5276] text-white py-2 px-6 rounded hover:bg-[#0e3040] transition-colors font-bold">
                    REGISTRAR
                </button>
            </div>
        </form>
...
// src/pages/PregnantRegisterPage.tsx
import React, { useState, useEffect } from 'react';
...
// --- A interface IGestante continua a mesma ---
interface IGestante {
  id: number;
  cpf: string;
  nome: string;
    ...
    <section className="bg-white p-6 rounded-lg shadow-md mb-8">
      ...
      <div className="flex flex-col sm:flex-row items-center gap-2 max-w-md mx-auto">
        <input
          type="text"
          value={cpf}
          // --- APLICA A MÁSCARA A CADA MUDANÇA ---
...
function RegistrationForm() {
  ...
  // --- 1. CRIAMOS UM "MAPA" PARA AS LEGENDAS PERSONALIZADAS ---
  ...
  // --- 2. CRIAMOS UMA LISTA EXPLÍCITA DE CAMPOS DE DATA ---
  //    Isso é mais robusto do que verificar se a palavra 'data' existe no nome.
    ...
    <section className="bg-white p-6 rounded-lg shadow-md mb-8">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {Object.keys(formData).map((key) => (
          <div key={key} className="flex flex-col">
            {/* --- 3. USAMOS O MAPA DE LEGENDAS AQUI --- */}
            <label htmlFor={key} className="mb-1 font-semibold text-gray-700">
              {labels[key]}:
            </label>
            <input
              // --- 4. USAMOS A LISTA DE DATAS AQUI ---
